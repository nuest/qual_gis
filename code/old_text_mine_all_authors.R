#Titel: spatial textmining all authors
...
print("spatial textmining - all authors")
#************
#Packages---------
#************
...
db <- "E:/GoogleDrive/Studium/qual_gis/Abgabe_final/Datenbank/Database_WOS.accdb"
...
colnames(locAbs)[5] <- "c" #for binding

# country colum c2 - c4 
c2 <- data.frame(y = locAbs$y, t= locAbs$t, a = as.character(locAbs$a), w=locAbs$w, c = locAbs$c.2)
c3 <- data.frame(y = locAbs$y, t= locAbs$t, a = as.character(locAbs$a), w=locAbs$w, c = locAbs$c.3)
c4 <- data.frame(y = locAbs$y, t= locAbs$t, a = as.character(locAbs$a), w=locAbs$w, c = locAbs$c.4)

locAbs[8] <- NULL  # deleting columns for rbind
locAbs[7] <- NULL
locAbs[6] <- NULL

locAbs <- rbind(locAbs, c2) #binding c2-c4
locAbs <- rbind(locAbs, c3)
locAbs <- rbind(locAbs, c4)
...
#rename countryIDs-----
#*****************
locAbs[,"c"][locAbs[,"c"] == "1"]<- "United States"  # to patch: Add a list with iso3 codes of each country 
...
LocAbs_words <- clean_text %>%
  unnest_tokens(word,a)%>%    # each word in a new line
  filter(str_detect(word, "[a-z']$"),
         !str_detect(word,"^research"),  # better solution: add a list as addition to the stopword list
...
total_titel <- clean_text %>%    # titel per year
  group_by(t) %>%
  count(c,t, sort = TRUE) %>%
  group_by(c) %>%
...
LocAbs_words <- left_join(LocAbs_words, geo)              # problem geocode in for-loop -> slow 
...
#tf-idf-------------------------------
#**********************************************************
# tf-idf for each word grouped by country id
...
LocAbs_cors <- tf_idf %>%
  filter(total_titel > 2)%>%  # more then 2 Paper per country for cor, to exclude cor between same paper from dif.
  filter(tf_idf > 0.0009) %>% # filter tf_idf to remove unimportant words
  pairwise_cor(country, word,n, sort = TRUE)
print("cor done")

#new word count with tf_idf filter
ttl_word2<- tf_idf %>%
...
df <- data.frame(from = LocAbs_cors$country,           # df --> holds the relevant info for the graph
...
df$weight[df$correlation <0.4] <-0.2     # for line thickness control
...
print("df done")
meta <- data.frame(name = LocAbs_cors$country, # meta for the spatial layout of the graph
                   lon = LocAbs_cors$lon,
                   lat = LocAbs_cors$lat)

meta <- meta[complete.cases(meta),]
meta <- meta[!duplicated(meta),]  
                                  # vector of missing countries
lost <- data_frame(name = c("Ireland", "Czech Republic", "Denmark", "Finland","Austria", "Bangladesh", "Belgium", "Italy"),
                   lon = c(-7.651600,15.268294,9.150484,25.768525,15.617265, 89.80256, 4.731598, 12.101991  ),
                   lat = c(53.422047,49.863959, 55.656755, 62.698190, 47.907434,23.136735, 50.708669,43.501196))
meta <- rbind(meta, lost)

colnames(total_titel)[1] <- "name"
meta <- left_join(meta, total_titel)  
...
#### End of the for-loop --> if condition controls the data per year

if(y == 1){
  gg10 <- get.data.frame(g, "both") #information for verticies & edges
  vert10 <- gg10$vertices
  coordinates(vert10) <- ~lon+lat # verticies done, just use the coord. to plot each point
  #*******
  # data 2010------
  #*******
  edges10 <- gg10$edges                                     # transformation to spatial object needed
  
  edges10 <- lapply(1:nrow(edges10), function(i) {
    as(rbind(vert10[vert10$name == edges10[i, "from"], ],  # connection between each vert. = edge 
             vert10[vert10$name == edges10[i, "to"], ]), 
       "SpatialLines")
  })
  for (i in seq_along(edges10)) {
    edges10[[i]] <- spChFIDs(edges10[[i]], as.character(i)) # transformation to spatial object
  }
  edges10 <- do.call(rbind, edges10) # join spatial info & data in gg10
  #****
...
write.csv2(total_titel, "total_titel.csv")
...
tf_idf %>%
  ...
  summarize(total = sum(n)) %>%
  top_n(10, wt = total) %>%
  ggplot(aes(reorder(word,total), total, fill = total)) + geom_histogram(stat="identity", show.legend = FALSE)+
  scale_fill_gradient(high="red", low="grey") +
  coord_flip() + xlab("") + ylab("")+
  geom_text(aes(label = total), size = 4.5, alpha= 0.6, hjust = 1.6) +
  theme(axis.text.y = element_text(size = 11, angle = 0), axis.text.x = element_text(size=11)) 
...
plt_tf2 <-tf_idf %>%
  ...
  filter(country == "Finland" | country == "United Kingdom" | country == "Canada") %>%
  ggplot(aes(n)) + geom_histogram(bins = 80,fill = "indianred") +
  facet_wrap(~country)  +  theme(strip.text.x = element_text(size = 14)) +
  scale_fill_gradient(high="red", low="grey") + xlab("Worthäufigkeiten") + ylab("")+
  theme(axis.text.y = element_text(size = 12, angle = 0), axis.text.x = element_text(size=12), axis.title = element_text(size = 14)) 

plt_tf2


tf_idf %>%
  select(country,word,tf_idf)%>%
  filter(country == "United States"| country == "Canada"
         | country == "Finland" |country == "United Kingdom" |
           country == "Australia")%>%
  arrange(desc(tf_idf)) %>%
  ...
  top_n(10) %>%
  ungroup() %>%
  ggplot(aes(reorder(word, tf_idf),tf_idf, fill = tf_idf)) + geom_col(show.legend = FALSE) +
  coord_flip()+ ylab("TF-IDF") + xlab("")+
  scale_fill_gradient(high="red", low="grey")+
  facet_wrap(~country, scales = "free_y") + theme(strip.text.x = element_text(size = 14)) +
  theme(axis.text.y = element_text(size = 12, angle = 0), axis.text.x = element_text(size=12)) 
...
writeOGR(lines, "lines17","lines17", driver = "ESRI Shapefile")

gg_points <- gg17$vertices %>%
  select(lon, lat, total_titel, name)

points <- SpatialPointsDataFrame(vert17, data = gg_points)
writeOGR(points, "points17","points17", driver = "ESRI Shapefile")